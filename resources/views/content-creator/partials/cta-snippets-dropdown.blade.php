{{-- 
    ...
    Self-contained Alpine component for selecting/managing CTA snippets.
    Uses entangle to sync with parent component if available.
    
    Required parent data properties:
    - cta: string (the selected CTA)
    - cta_snippets: array (the list of snippets)
    - newSnippet: string (for adding new snippets)
    - addSnippet(): function
    - removeSnippet(index): function
--}}
<div class="relative" 
     x-data="{ 
         showDropdown: false, 
         ...
         // These sync with parent via $parent or use local fallback
         get snippets() { 
             return this.$data.cta_snippets ?? []; 
         },
         
         selectSnippet(snippet) {
             if (typeof this.$data.cta !== 'undefined') {
                 this.$data.cta = snippet;
             }
    ...
    <div x-show="showDropdown" 
        ...
        <div class="p-2 bg-muted/50 border-b border-border flex items-center justify-between px-3 shrink-0">
            <span class="text-[9px] font-black uppercase tracking-widest text-muted-foreground" 
                  x-text="isManaging ? 'Manage Snippets' : 'Quick Select'"></span>
            <button @click="isManaging = !isManaging" 
                    class="text-primary hover:text-primary/80 transition-colors" 
                    title="Manage Snippets">
                <i :class="{ 'fill-current': isManaging }" data-lucide="settings-2" class="w-3 h-3"></i>
            </button>
        </div>
        ...
        <div class="max-h-56 overflow-y-auto">
            {{-- Select Mode --}}
            <template x-if="!isManaging">
                <div>
                    <template x-for="snippet in cta_snippets" :key="snippet">
                        <button @click="selectSnippet(snippet)" type="button" 
                    ...
                    <div x-show="!cta_snippets || cta_snippets.length === 0" 
