...
<div x-show="showBulkScheduleModal" 
     x-cloak
     class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" 
     x-transition>
    
    <div @click.away="showBulkScheduleModal = false" 
        ...
        <div class="space-y-2">
            ...
            <input type="date" x-model="bulkStartDate" 
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <label class="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/10 cursor-pointer hover:bg-muted/20 transition-all" 
                       :class="bulkPlatforms.includes('facebook') ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-600/20' : ''">
                    <input type="checkbox" value="facebook" x-model="bulkPlatforms" class="hidden">
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/10 cursor-pointer hover:bg-muted/20 transition-all" 
                       :class="bulkPlatforms.includes('linkedin') ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500/20' : ''">
                    <input type="checkbox" value="linkedin" x-model="bulkPlatforms" class="hidden">
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/10 cursor-pointer hover:bg-muted/20 transition-all" 
                       :class="bulkPlatforms.includes('instagram') ? 'border-pink-500 bg-pink-50 ring-1 ring-pink-500/20' : ''">
                    <input type="checkbox" value="instagram" x-model="bulkPlatforms" class="hidden">
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/10 cursor-pointer hover:bg-muted/20 transition-all" 
                       :class="bulkPlatforms.includes('twitter') ? 'border-sky-500 bg-sky-50 ring-1 ring-sky-500/20' : ''">
                    <input type="checkbox" value="twitter" x-model="bulkPlatforms" class="hidden">
        ...
        <div class="flex gap-3 pt-2">
            <button @click="showBulkScheduleModal = false" 
            ...
            <button @click="confirmBulkSchedule" 
                    :disabled="isBulkScheduling || bulkPlatforms.length === 0"
                ...
                <template x-if="!isBulkScheduling">
                    <i data-lucide="check" class="w-4 h-4"></i>
                </template>
                <template x-if="isBulkScheduling">
                    <i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i>
                </template>
                <span x-text="isBulkScheduling ? 'Scheduling...' : 'Confirm Schedule'"></span>
            </button>
        </div>
    </div>
</div>
