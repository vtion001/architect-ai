...
<div x-show="$store.cc.showSuccessModal" 
     x-cloak
    ...
    <div @click.away="$store.cc.showSuccessModal = false" 
        ...
        <p class="text-muted-foreground mb-6 leading-relaxed text-sm font-medium relative z-10">
            <span x-show="$store.cc.generator === 'blog' && $store.cc.isBatchMode && $store.cc.batchChildren.length > 0">
                Your blog batch is ready — <span x-text="$store.cc.batchChildren.length"></span> posts generated.
            </span>
            <span x-show="!($store.cc.generator === 'blog' && $store.cc.isBatchMode && $store.cc.batchChildren.length > 0)">
        ...
        <div x-show="$store.cc.generator === 'blog' && $store.cc.isBatchMode && $store.cc.batchChildren.length > 0" 
             class="relative z-10 text-left mb-6 space-y-3 max-h-80 overflow-y-auto">
            <template x-for="(child, idx) in $store.cc.batchChildren" :key="child.id">
        ...
        <div class="flex flex-col gap-3 relative z-10">
            <a :href="'/content-creator/' + $store.cc.createdContentId" 
               class="w-full h-12 rounded-xl bg-primary text-primary-foreground font-black uppercase tracking-widest text-xs hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2">
                ...
                <span x-text="$store.cc.generator === 'blog' && $store.cc.isBatchMode ? 'View Full Batch' : 'View Content'"></span>
            </a>
            <button @click="$store.cc.showSuccessModal = false; $store.cc.batchChildren = []" 
