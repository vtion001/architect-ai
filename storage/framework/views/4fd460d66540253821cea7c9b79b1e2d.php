...
<div class="grid grid-cols-2 gap-3" x-show="!showVariantModal">
    <?php $__currentLoopData = $templateCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <button
        @click="
            selectedCategory = <?php echo \Illuminate\Support\Js::from($category)->toHtml() ?>;
            template = '<?php echo e($category['id']); ?>';
            showVariantModal = true;
        "
        class="flex flex-col items-start p-3 rounded-xl border text-left transition-all hover:shadow-md"
        :class="template === '<?php echo e($category['id']); ?>' 
            ? 'border-primary bg-primary/5 ring-2 ring-primary/20' 
            : 'border-border bg-card hover:border-primary/50'"
    >
        <div class="w-full flex items-start justify-between">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-3 transition-transform"
                 style="background-color: <?php echo e($category['color']); ?>15; color: <?php echo e($category['color']); ?>"
                 :class="template === '<?php echo e($category['id']); ?>' ? 'scale-110 shadow-lg' : ''">
                <i data-lucide="<?php echo e($category['icon']); ?>" class="w-5 h-5"></i>
            </div>
            <i data-lucide="chevron-right" class="w-4 h-4 transition-colors"
               :class="template === '<?php echo e($category['id']); ?>' ? 'text-primary' : 'text-muted-foreground/50'"></i>
        </div>
        <span class="text-[13px] font-bold leading-tight"
              :class="template === '<?php echo e($category['id']); ?>' ? 'text-primary' : 'text-muted-foreground'">
...
<?php /**PATH /var/www/resources/views/components/template-selector.blade.php ENDPATH**/ ?>