...
<div class="p-8 max-w-7xl mx-auto" x-data="{ 
    ...
    async fetchSuggestions() {
        ...
        this.suggestions = '';
        try {
            const response = await fetch('<?php echo e(route('social-planner.suggestions')); ?>', {
                method: 'POST',
                headers: {
        ...
        <div @click.away="showCreatePostModal = false" class="bg-card text-card-foreground w-full max-w-2xl rounded-xl shadow-2xl flex flex-col border border-border max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <textarea class="w-full h-32 bg-muted/20 border border-border rounded-xl p-4 text-sm font-medium focus:ring-1 focus:ring-primary resize-none" placeholder="Write your caption here..."></textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                     <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-foreground italic">Date</label>
                        <input type="date" class="w-full h-12 bg-muted/20 border border-border rounded-xl px-4 text-sm font-medium">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-foreground italic">Time</label>
                        <input type="time" class="w-full h-12 bg-muted/20 border border-border rounded-xl px-4 text-sm font-medium">
                    </div>
                </div>

                <div class="flex justify-end pt-4">
                    <button class="bg-primary text-primary-foreground px-6 py-2 rounded-lg text-sm font-bold hover:bg-primary/90">
                        Schedule Post
                    </button>
                </div>
            </div>
