...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        const config = {
            deleteUrl: '<?php echo e(route("content-creator.destroy", $content->id)); ?>',
            redirectUrl: '<?php echo e(route("content-creator.index")); ?>',
        ...
        Alpine.data('postCard', (index, initialRaw, initialHtml, alreadyPublished = false) => ({
            ...
            init() {
                const visuals = <?php echo \Illuminate\Support\Js::from($content->options['visuals'] ?? [])->toHtml() ?>;
            ...
            persistVisual(idx) {
                fetch(`/content-creator/<?php echo e($content->id); ?>/save-visual`, {
                    method: 'POST',
        ...
        <div x-data="postCard(<?php echo e($index); ?>, <?php echo e(Js::from($finalPostContent)); ?>, <?php echo e(Js::from($cleanHtml)); ?>, <?php echo e(in_array($index, $publishedIndexes) ? 'true' : 'false'); ?>)" 
            ...
            <div x-show="isPublished" x-transition.opacity class="absolute inset-0 z-40 bg-white/90 backdrop-blur-[2px] flex flex-col items-center justify-center p-6 text-center" style="display: none;">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="<?php echo e(route('social-planner.index')); ?>" class="px-4 py-2 text-xs font-bold uppercase tracking-widest bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-all shadow-md">
                        View in Planner
                    </a>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/content-creator/content-viewer.blade.php ENDPATH**/ ?>